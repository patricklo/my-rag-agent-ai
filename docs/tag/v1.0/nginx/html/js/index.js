...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const loadRagOptions = () => {
        ...
        fetch('http://114.132.178.157:8090/my-rag-agent-ai/api/v1/rag/query_rag_tag_list')
...
function appendMessage(content, isAssistant = false, saveToStorage = true) {
 ...
 function startEventStream(message) {
    ...
    if (ragTag) {
        url = `http://114.132.178.157:8090/my-rag-agent-ai/api/v1/${aiModelValue}/generate_stream_rag?message=${encodeURIComponent(message)}&ragTag=${encodeURIComponent(ragTag)}&model=${encodeURIComponent(aiModelModel)}`;
    } else {
        url = `http://114.132.178.157:8090/my-rag-agent-ai/api/v1/${aiModelValue}/generate_stream?message=${encodeURIComponent(message)}&model=${encodeURIComponent(aiModelModel)}`;
    }
